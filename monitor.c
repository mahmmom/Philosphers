...
/*   Updated: 2024/01/21 19:28:41 by mohamoha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	philo_died(t_philo	*philo)
{
	...
	if (get_bool(&philo->pilo_mutex, &philo->full))
		return (false);
	elapsed = get_time_day(MILLISECOND) - get_long(&philo->pilo_mutex, &philo->last_meal_time);
	...
	while (!rotuine_finished(data))
	{
		...
		while (i < data->philo_num)
		{
			if (philo_died(data->philos + i) && !rotuine_finished(data))
			{
				...
				philo_status(DIED, data->philos + i, DEBUGGER);
			}
			i++;
		}
	}
